...
import { renderSpriteGallery } from "./sprite-gallery.ts";
...
/**
 * Route dispatcher. The default route boots the game; the `/sprites` route
 * mounts a read-only character/boss gallery without touching the game loop
 * or audio. Vite's default `appType: "spa"` serves `index.html` for unknown
 * paths in dev and `vite preview`; static-host deployments need their own
 * SPA fallback for `/sprites` to resolve.
 */
function isSpriteGalleryRoute(): boolean {
  const p = window.location.pathname;
  return p === "/sprites" || p === "/sprites/" || p.endsWith("/sprites");
}

if (isSpriteGalleryRoute()) {
  document.title = "Pactkeeper — Sprite Gallery";
  canvasEl.hidden = true;
  pactEl.hidden = false;
  renderSpriteGallery(pactEl);
} else {
  bootGame();
}

/**
 * Wire up the game + pact screen and choose the opening stage. Extracted
 * into a function so the `/sprites` branch above can short-circuit cleanly
 * (no game loop, no audio, no PactScreen instance).
 */
